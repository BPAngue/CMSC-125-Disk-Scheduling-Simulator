...
public class LOOK implements PausableSimulator, Runnable {
    ...
    public LOOK(Simulator simulator, CartesianPanel cartesian, ArrayList<Integer> diskQueue, ArrayList<Movement> headMovements, int headLocation, String directionOfMovement, int simulationSpeed, SimulationContext simulation) {
    ...
    private void updateSeekTimeLabel() {
        if (simulator.getAlgorithm().equalsIgnoreCase("ALL")) {
            simulation.updateSeekTimeLabelLook(totalSeekTime);
        } else {
    ...
    private void updateSimulationClock() {
        ...
        if (simulator.getAlgorithm().equalsIgnoreCase("ALL")) {
            simulation.updateTimerLabelLook(minutes, seconds);
        } else {
