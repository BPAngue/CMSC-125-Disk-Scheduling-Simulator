...
public class CSCAN implements PausableSimulator, Runnable {
    ...
    private void updateSeekTimeLabel() {
        if (simulator.getAlgorithm().equalsIgnoreCase("ALL")) {
            simulation.updateSeekTimeLabelCscan(totalSeekTime);
        } else {
    ...
    private void updateSimulationClock() {
        ...
        if (simulator.getAlgorithm().equalsIgnoreCase("ALL")) {
            simulation.updateTimerLabelCscan(minutes, seconds);
        } else {
